        ...
        case "\t": // TAB
          if (this._autocompleteHandlers.length > 0) {
            ...
            if (candidates.length === 0) {
              ...
              const lastToken = getLastToken(inputFragment).split("/").pop();
              this.handleCursorInsert(
                candidates[0].substr(lastToken.length) + (candidates[0].endsWith("/") ? "" : " ")
              );
            } else if (candidates.length <= this.maxAutocompleteEntries) {

              // search for a shared fragement
              const sameFragment = getSharedFragment(inputFragment.split("/").pop(), candidates);
